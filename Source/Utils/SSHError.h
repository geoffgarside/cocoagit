...
// Define SSHError* macros to use the OmniBase _OBError helper functions. If we decide to move away from OmniBase code, we can just redfine these.
#define SSHErrorWithDescription(error, code, description, ...) _OBErrorWithDescription(error, SSHErrorDomain, code.val, __FILE__, __LINE__, description) 
#define SSHError(error, code, description) _OBError(error, SSHErrorDomain, code, __FILE__, __LINE__, NSLocalizedDescriptionKey, description, nil)
#define SSHErrorWithInfo(error, code, ...) _OBError(error, SSHErrorDomain, code, __FILE__, __LINE__, ## __VA_ARGS__)
