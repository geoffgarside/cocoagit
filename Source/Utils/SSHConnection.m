...
+ (NSError *)makeErrorWithCode : (int)code message : (NSString *)message {
    NSDictionary *userInfo =
        [NSDictionary dictionaryWithObjectsAndKeys: @"The file could not be transferred.",
...
// generates an NSError based on the last error reported by libssh2, preceded by
...
- (NSError *)connectToHost: (NSString *)newHost port: (unsigned short)newPort user: (NSString *)
user password: (NSString *)password {
    int err;
    ...
    if ( ![socketPort address] )
        return [SSHConnection makeErrorWithCode: SSHCONNECTION_ERROR_LOOKUP message:[NSString
                                                                                     stringWithFormat
                                                                                     :
                                                                                     @"Could not look up the hostname (%@).", newHost]];

    // establish connection
    const struct sockaddr *addr = [[socketPort address] bytes];
    sock = socket(addr->sa_family, SOCK_STREAM, 0);
    if ( sock == -1 )
        return [SSHConnection makeErrorWithCode: errno message:[NSString stringWithFormat:
                                                                @"Socket could not be opened for connection (%i).",
                                                                errno]];
    ...
    if ( err )
        return [SSHConnection makeErrorWithCode: errno message:[NSString stringWithFormat:
                                                                @"Connection failed (%i).",errno]];

    // establish SSH session
    session = libssh2_session_init();
    if ( libssh2_session_startup(session, sock)) {
        [self disconnect];
        return [SSHConnection makeErrorWithCode: SSHCONNECTION_ERROR_SESSION message:
    ...
    if ( libssh2_userauth_password(session, CSTR(user), CSTR(password))) {
        [self disconnect];
        return [SSHConnection makeErrorWithCode: SSHCONNECTION_ERROR_AUTH message:
...
- (NSError *)connectToHost: (NSString *)newHost user: (NSString *)user password: (NSString *)
password {
...
+ (SSHConnection *)connectionToURL: (NSURL *)url error: (NSError **)errorOut {
    ...
    NSError *err =
        [conn connectToHost:[[url host] stringByReplacingPercentEscapesUsingEncoding:
                             NSUTF8StringEncoding
         ]
         user:[[url user] stringByReplacingPercentEscapesUsingEncoding:
               NSUTF8StringEncoding]
         password:[[url password] stringByReplacingPercentEscapesUsingEncoding:
                   NSUTF8StringEncoding]];
    if ( err && errorOut ) {
        *errorOut = err;
        return NULL;
...
- (NSError *)writeData: (NSData *)data toPath: (NSString *)destPath {
    if ( !session )
        return [SSHConnection makeErrorWithCode: SSHCONNECTION_ERROR_NOSESSION message:
                @"Data can't be written because no session is open."];

    // open scp channel
    LIBSSH2_CHANNEL *channel = libssh2_scp_send(session, CSTR(destPath), 0666, [data length]);
    if ( !channel ) {
        NSError *err =
            [SSHConnection libssh2ErrorForSession: session message:
             @"scp channel initialization failed"
            ];
        [self disconnect];
        return err;
    }
    ...
    if ( written != [data length] )
        return [SSHConnection libssh2ErrorForSession: session message:
                @"Some data could not be written"];
...
+ (NSError *)scpData: (NSData *)data to: (NSURL *)url {
    ...
    err =
        [conn writeData: data toPath:[[url path] stringByTrimmingCharactersInSet:[NSCharacterSet
...
- (NSError *)deleteFileAtPath: (NSString *)path {
    ...
    if ( !session )
        return [SSHConnection makeErrorWithCode: SSHCONNECTION_ERROR_NOSESSION message:
                @"File cannot be deleted because no session is open."];
...
+ (NSError *)deleteFileAtURL: (NSURL *)url {
    ...
    err =
        [conn deleteFileAtPath:[[url path] stringByTrimmingCharactersInSet:[NSCharacterSet
                                                                            characterSetWithCharactersInString
                                                                            : @"/"]]];
    [conn disconnect];
