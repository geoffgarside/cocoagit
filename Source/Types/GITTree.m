...
- (BOOL)parseRawData: (NSData *)raw error: (NSError**)error {
    ...
    do {
        ...
        GITTreeEntry *entry =
            [[GITTreeEntry alloc] initWithRawString: treeLine parent: self error: &undError];

        if ( !entry ) {
            errorDescription = NSLocalizedString(@"Failed to parse entry for tree",
                                                 @"GITErrorObjectParsingFailed (GITTree)");
            GITErrorWithInfo(error,
                             GITErrorObjectParsingFailed,
