...
- (GITCommit *)commit {
    if ( !commit && self.objectSha1 )
        self.commit = [self.repo commitWithSha1: objectSha1 error: NULL]; //!<
                                                                          // Ideally
                                                                          // we'd
                                                                          // like
                                                                          // to
...
- (BOOL)parseRawData: (NSData *)raw error: (NSError**)error {
    ...
    *taggerTimezone;
        ...
        [taggedType isEqualToString: @"commit"] )
    {
        ...
        errorDescription = NSLocalizedString(@"Failed to parse object (commit) reference for tag",
                                             @"GITErrorObjectParsingFailed (GITTag:object)");
    ...
    if ([scanner scanString: @"tag" intoString: NULL] &&
        ...
        errorDescription = NSLocalizedString(@"Failed to parse name for tag",
                                             @"GITErrorObjectParsingFailed (GITTag:tag)");
        GITError(error, GITErrorObjectParsingFailed, errorDescription);
        return NO;
    }
        ...
        [scanner scanUpToString: NewLine intoString: &taggerTimezone] )
    {
        ...
        errorDescription = NSLocalizedString(@"Failed to parse tagger for tag",
                                             @"GITErrorObjectParsingFailed (GITTag:tagger)");
        GITError(error, GITErrorObjectParsingFailed, errorDescription);
        return NO;
    }
    ...
    if ( !self.message ) {
        errorDescription = NSLocalizedString(@"Failed to parse message for tag",
                                             @"GITErrorObjectParsingFailed (GITTag:message)");
        GITError(error, GITErrorObjectParsingFailed, errorDescription);
        return NO;
    }
