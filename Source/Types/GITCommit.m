...
- (GITTree *)tree {
    if ( !tree && self.treeSha1 )
        self.tree = [self.repo treeWithSha1: self.treeSha1 error: NULL]; //!<
                                                                         // Ideally
                                                                         // we'd
                                                                         // like
                                                                         // to
...
- (GITCommit *)parent {
    if ( !parent && self.parentSha1 )
        self.parent = [self.repo commitWithSha1: self.parentSha1 error: NULL];  //!<
                                                                                // Ideally
                                                                                // we'd
                                                                                // like
                                                                                // to
...
- (BOOL)parseRawData: (NSData *)raw error: (NSError**)error {
    ...
    *committerTimezone;
        ...
        [scanner scanUpToString: NewLine intoString: &commitTree] )
    {
        ...
        errorDescription = NSLocalizedString(@"Failed to parse tree reference for commit",
                                             @"GITErrorObjectParsingFailed (GITCommit:tree)");
    ...
    while ([scanner scanString: @"parent" intoString: NULL] ) {
        ...
        if ([scanner scanUpToString: NewLine intoString: &commitParent] ) {
            ...
            errorDescription = NSLocalizedString(@"Failed to parse parent reference for commit",
                                                 @"GITErrorObjectParsingFailed (GITCommit:parent)");
            GITError(error, GITErrorObjectParsingFailed, errorDescription);
            return NO;
        }
    }
        ...
        [scanner scanUpToString: NewLine intoString: &authorTimezone] )
    {
        ...
        errorDescription = NSLocalizedString(@"Failed to parse author details for commit",
                                             @"GITErrorObjectParsingFailed (GITCommit:author)");
        GITError(error, GITErrorObjectParsingFailed, errorDescription);
        return NO;
    }
        ...
        [scanner scanUpToString: NewLine intoString: &committerTimezone] )
    {
        ...
        errorDescription = NSLocalizedString(@"Failed to parse committer details for commit",
                                             @"GITErrorObjectParsingFailed (GITCommit:committer)");
        GITError(error, GITErrorObjectParsingFailed, errorDescription);
        return NO;
    }
    ...
    if ( !self.message ) {
        errorDescription = NSLocalizedString(@"Failed to parse message for commit",
                                             @"GITErrorObjectParsingFailed (GITCommit:message)");
        GITError(error, GITErrorObjectParsingFailed, errorDescription);
        return NO;
    }
