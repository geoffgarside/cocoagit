...
@interface GITTreeEntry : NSObject
{
    ...
    GITObject *object;  //!< The object which is referenced. This is lazy
                        // loaded.
}
...
/*! Creates and returns a new entry by extracting the fields from the raw
 * string.
 * \param raw The raw string from a tree object file
 * \param parent The parent tree this entry belongs to
 * \param[out] error Error containing a description of any errors if they
 *occurred
 * \return A new entry or nil if an error occurred
 * \par Errors:
 * \li \c GITErrorObjectParsingFailed indicates a problem parsing the formatted
 *tree entry
 */
 ...
 */
- (id)initWithFileMode: (NSUInteger)mode name: (NSString *)name sha1: (NSString *)sha1 parent: (
    GITTree *)parent error: (NSError**)error;
