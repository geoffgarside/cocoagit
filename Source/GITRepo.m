...
- (id)initWithRoot: (NSString *)repoRoot bare: (BOOL)isBare error: (NSError**)error {
    ...
    objectStore =
        [[[GITCombinedStore alloc] initWithStores: fileStore, packStore, nil] autorelease];
...
- (GITObject *)objectWithSha1: (NSString *)sha1 type: (GITObjectType)eType error: (NSError*
                                                                                   *)error {
    GITObjectType type;
    NSData *data;
    if ( ![self.store loadObjectWithSha1: sha1 intoData: &data type: &type error: error] ) {
        GITError(error, GITErrorObjectNotFound,
                 NSLocalizedString(@"Object not found", @"GITErrorObjectNotFound"));
        return nil;
    }

    if ( !(eType == GITObjectTypeUnknown || eType == type)) {
        GITError(error, GITErrorObjectTypeMismatch,
                 NSLocalizedString(@"Object type mismatch", @"GITErrorObjectTypeMismatch"));
        return nil;
    }
    ...
    // If we get here, then we've got a type that we don't understand. If the
    // only way this could happen is a programming error, then it should be an
    // exception.  For now, just create an error.
    GITError(error, GITErrorObjectTypeMismatch,
             NSLocalizedString(@"Object type mismatch", @"GITErrorObjectTypeMismatch"));
...
- (NSArray *)branches {
    ...
    for ( NSDictionary *ref in refs ) {
        ...
        branch.name = [refName lastPathComponent]; // probably should explicitly
                                                   // split on slashes
        branch.repo = self;
...
- (NSArray *)refs {
        ...
        while ( (thisRef = [e nextObject]) ) {
            ...
            if ([fm fileExistsAtPath: tempRef isDirectory: &isDir] && !isDir ) {
                NSString *shaString = [[NSString alloc] initWithContentsOfFile: tempRef
                                       ...
                                       error: nil];
                thisSha =
                    [shaString stringByTrimmingCharactersInSet:[NSCharacterSet newlineCharacterSet]
                    ];
                [shaString release];
    ...
    if ([fm fileExistsAtPath: packedRefsPath] ) {
        NSString *packedRefs = [[NSString alloc] initWithContentsOfFile: packedRefsPath
                                encoding: NSASCIIStringEncoding
                                error: nil];
        NSArray *packedRefLines =
            [packedRefs componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]];
    ...
    NSString *headRefContents = [[NSString alloc] initWithContentsOfFile:[self headRefPath]
                                 encoding: NSASCIIStringEncoding
                                 error: nil];
    NSString *headRefTemp =
        [headRefContents stringByTrimmingCharactersInSet:[NSCharacterSet newlineCharacterSet]];
...
- (NSString *)pathForLooseObjectWithSha: (NSString *)shaValue {
    ...
    return [NSString stringWithFormat: @"%@/objects/%@/%@", [self root], looseSubDir,
            looseFileName];
}
...
- (BOOL)writeObject: (NSData *)objectData withType: (NSString *)type size: (NSUInteger)size {
    ...
    //NSData *compress = [[NSData dataWithBytes:[object bytes] length:[object
    ...
    // return a string? Should probably return a BOOL to indicate that file has
    // been written...
    return success;
}
