...
- (NSData *)unpackObjectWithSize: (int)size compressedSize: (int *)cSize compressedData: (NSData *
                                                                                          *)cData;
- (NSData *)unpackDeltaObjectWithSize: (NSUInteger)size type: (NSUInteger)type compressedData: (
    NSData **)cData;
@end
...
- (NSString *)capabilitiesWithPacket: (NSData *)data;
{
    ...
    return [[[NSString alloc] initWithData: capData encoding:[NSString defaultCStringEncoding]]
            autorelease];
}
...
- (NSDictionary *)readPackHeader;
{
    ...
    return [NSDictionary dictionaryWithObjectsAndKeys: header, @"data",
            [NSNumber numberWithUnsignedInt: entries], @"entries", nil];
}
...
- (NSData *)readPackObject;
{
    ...
    // We do this, because packfiles do not provide the size of the compressed
    // object
    ...
    NSData **)cData;
{
    ...
    if ( type == kGITPackFileTypeDeltaRefs )
        ...
        // TODO: This is currently broken.  I'm not sure I fully understand
        // Delta Offsets yet
                                                                                          ...
                                                                                          *)cData;
{
    ...
    while ( !done ) {
        ...
        if ( zstatus == Z_STREAM_END ) done = YES;
            ...
            NSLog(@"status for break: %d, next_out = %x, avail_out = %d, total = %d",
                  zstatus,
                  strm.next_out,
                  strm.avail_out,
                  strm.total_out);
