...
- (void)handleRequest {
    ...
    if ([command isEqualToString: @"git-receive-pack"] )                // git
                                                                        // push
                                                                        //  //
        [self receivePack: repository];
    else if ([command isEqualToString: @"git-upload-pack"] )            // git
                                                                        // fetch
                                                                        // //
...
- (void)sendPackData {
    ...
    while ( (current = [e nextObject]) ) {
        ...
        // pack object data
        //objData = [NSData dataWithBytes:[obj rawContents] length:([obj
        // rawContentLen])];
...
- (void)unpackObject {
    ...
    if ((type == GITObjectTypeCommit) || (type == GITObjectTypeTree) ||
        (type == GITObjectTypeBlob) || (type == GITObjectTypeTag)) {
...
- (NSArray *)patchDeltaHeaderSize: (NSData *)deltaData position: (unsigned long)position {
    ...
    return [NSArray arrayWithObjects:[NSNumber numberWithLong: size],
            [NSNumber numberWithLong: position], nil];
}
...
- (NSString *)packetReadLine {
    ...
    NSString *packetLine =
        [[NSString alloc] initWithBytes: data length: len encoding: NSASCIIStringEncoding];
