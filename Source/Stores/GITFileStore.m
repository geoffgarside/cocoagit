...
- (id)initWithRoot: (NSString *)root error: (NSError**)error {
    ...
    if ( ! [fm fileExistsAtPath: self.objectsDir isDirectory: &aDirectory] || !aDirectory ) {
        NSString *errFmt = NSLocalizedString(
            @"File store not accessible %@ does not exist or is not a directory",
            @"GITErrorObjectStoreNotAccessible (GITFileStore)");
...
type: (GITObjectType *)type error: (NSError**)error {
    ...
    if ( ![fm isReadableFileAtPath: path] ) {
        NSString *errorDescription =
            [NSString stringWithFormat: NSLocalizedString(@"Object %@ not found",
                                                          @"GITErrorObjectNotFound"), sha1];
    ...
    *type = [GITObject objectTypeForString: typeStr];
    ...
    // We could check *data and *type individually, and bail with a more
    // specific error if they are nil.
