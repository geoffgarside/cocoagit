...
- (id)initWithRoot: (NSString *)root error: (NSError**)error {
    ...
    if ( ! [fm fileExistsAtPath: self.packsDir isDirectory: &aDirectory] || !aDirectory ) {
        NSString *errFmt = NSLocalizedString(
            @"PACK store not accessible %@ does not exist or is not a directory",
            @"GITErrorObjectStoreNotAccessible (GITPackStore:init)");
...
- (NSArray *)loadPackFilesWithError: (NSError**)outError {
    ...
    if ( !files ) {
        NSString *errFmt = NSLocalizedString(
            @"PACK store not accessible, load packs from %@ failed",
            @"GITErrorObjectStoreNotAccessible (GITPackStore:load)");
        NSString *errDesc = [NSString stringWithFormat: errFmt, self.packsDir];
    ...
    for ( NSString *file in files )
        if ([[file pathExtension] isEqualToString: @"pack"] ) {
            pack =
                [GITPackFile packFileWithPath:[self.packsDir stringByAppendingPathComponent: file]
                 error
                 : outError];
