 ...
 */
typedef enum {
    GITHighPriority = 0,    //!< Adds the store to the head of the list so it is
                            // checked first
 ...
 * The GITCombinedStore allows the user to combine a selection of GITObjectStore
 *subclasses into
 * a single store unit. The primary use would be for combining a GITFileStore
 *with a GITPackStore
 * so that both loose file objects and packed objects can be accessed through
 *one object.
 */
 ...
 * \param firstStore A variable nil terminated list of stores to add to the
 *receiver
 */
 ...
 * \param store The GITObjectStore instance to add to the receiver
 * \param priority The priority indicates where the list of stores the
 *<tt>store</tt>
 * should be placed. If you want the <tt>store</tt> to be checked first then you
 *can
