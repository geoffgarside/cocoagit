...
- (void)patchDeltaHeader: (NSData *)deltaData size: (unsigned long *)size position: (unsigned long
                                                                                     *)position;
{
    int shift = 0;
    unsigned char c;
...
- (NSData *)dataByPatchingWithDelta: (NSData *)deltaData;
{
    ...
    while ( position < [deltaData length] ) {
        ...
        if ( (c & 0x80) != 0 ) {
            ...
            [NSException raise: @"InvalidDeltaDataException" format:
             @"Delta data is invalid. Your packfile might be corrupt."];
