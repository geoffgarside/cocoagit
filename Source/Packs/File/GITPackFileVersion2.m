...
- (NSData *)unpackObjectAtOffset: (off_t)offset type: (GITObjectType *)objectType error: (NSError*
                                                                                          *)error;
- (NSData *)unpackDeltifiedObjectAtOffset: (off_t)offset type: (GITObjectType)deltaType
objectOffset: (off_t)objOffset objectType: (GITObjectType *)type error: (NSError**)error;
...
- (id)initWithData: (NSData *)packData error: (NSError **)error;
{
    ...
    if ( ! [self verifyChecksum] ) {
        NSString *errDesc = NSLocalizedString(@"PACK file checksum failed",
                                              @"GITErrorPackFileChecksumMismatch");
        GITErrorWithInfo(error,
                         GITErrorPackFileChecksumMismatch,
                         errDesc,
    ...
    switch ( type ) {
        ...
        case kGITPackFileTypeBlob:
            ...
            if ( objData && type && (size != [objData length])) {
                GITError(error, GITErrorObjectSizeMismatch,
                         NSLocalizedString(@"Object size mismatch", @"GITErrorObjectSizeMismatch"));
                return nil;
            }
            *objectType = type;
            break;
        ...
        case kGITPackFileTypeDeltaRefs:
            objData =
                [self unpackDeltifiedObjectAtOffset: offset type: type objectOffset: objOffset
                 objectType:
...
objectOffset: (off_t)objOffset objectType: (GITObjectType *)type error: (NSError**)error;
{
    ...
    if ( !baseObjectData ) {
        GITError(error, GITErrorObjectNotFound,
                 NSLocalizedString(@"Base Object not found for PACK delta",
                                   @"GITErrorObjectNotFound (GITPackFile)"));
        return nil;
    }
