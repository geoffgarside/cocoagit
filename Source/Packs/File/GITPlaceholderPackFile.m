...
- (id)initWithData: (NSData *)packData error: (NSError **)error;
{
    ...
    if ( memcmp(buf, kGITPackFileSignature, kGITPackFileSignatureRange.length) != 0 ) {
        ...
        // stringWithFormat:NSLocalizedString(@"Data is not valid PACK format",
        // "GITErrorPackFileInvalid"), thePath];
        GITError(error, GITErrorPackFileInvalid,
                 NSLocalizedString(@"Data is not valid PACK format", "GITErrorPackFileInvalid"));
        return nil;
    }
    ...
    switch ( ver ) {
        ...
        default:
            errorDescription =
                [NSString stringWithFormat: NSLocalizedString(@"Pack version %lu not supported",
                                                              @"GITErrorPackFileNotSupported"),
                 ver];
...
- (id)initWithPath: (NSString *)thePath error: (NSError **)error {
    ...
    if ( memcmp(buf, kGITPackFileSignature, kGITPackFileSignatureRange.length) != 0 ) {
        NSString *errorDescription =
            [NSString stringWithFormat: NSLocalizedString(@"File %@ is not a PACK file",
                                                          "GITErrorPackFileInvalid"), thePath];
