...
- (id)initWithPath: (NSString *)thePath error: (NSError**)outError {
    ...
    if ( ! [fileManager isReadableFileAtPath: thePath] ) {
        description =
            [NSString stringWithFormat: NSLocalizedString(@"File %@ not found",
                                                          @"GITErrorFileNotFound (GITPackIndex)"),
             thePath];
        ...
        default:
            description =
                [NSString stringWithFormat: NSLocalizedString(
                     @"Pack Index version %lu is not supported",
                     @"GITErrorPackIndexUnsupportedVersion"),
                 ver];
