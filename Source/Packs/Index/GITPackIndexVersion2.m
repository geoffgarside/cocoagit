...
- (id)initWithPath: (NSString *)thePath error: (NSError**)outError {
    ...
    if ( ! [self verifyChecksum] ) {
        NSString *errDesc = NSLocalizedString(@"PACK Index file checksum failed",
                                              @"GITErrorPackIndexChecksumMismatch");
        GITErrorWithInfo(outError,
                         GITErrorPackIndexChecksumMismatch,
                         errDesc,
    ...
    for ( i = 0; i < kGITPackIndexFanoutCount; i++ ) {
        ...
        if ( lastCount > thisCount ) {
            NSString *format = NSLocalizedString(
                @"Index: %@ : Invalid fanout %lu -> %lu for entry %d",
                @"GITErrorPackIndexCorrupted");
            NSString *reason =
                [NSString stringWithFormat: format, [self.path lastPathComponent], lastCount,
                 thisCount,
                 i];
...
//  - the fanout table tells you where in the main entry table you can find
// SHA's with a specific first byte
//  - the main sha1 list table gives you a sorted list of SHA1's in the Index
// and Pack file. The array index
//    of the SHA1 in this table equates to the array index of the pack offset in
// the offsets table.
...
- (off_t)packOffsetForSha1: (NSString *)sha1 error: (NSError**)error;
{
    ...
    // raise an error.
    NSString *errorFormat = NSLocalizedString(@"Object %@ is not in index file",
                                              @"GITErrorObjectNotFound");
...
- (BOOL)verifyChecksum {
    NSData *checkData =
        [[self.data subdataWithRange: NSMakeRange(0, [self.data length] - 20)] sha1Digest];
...
- (NSRange)rangeOfExtendedOffsetTable {
    NSUInteger endOfOffsetTable = [self rangeOfOffsetTable].location +
                                  [self rangeOfOffsetTable].length;
...
- (NSData *)packedSha1WithIndex: (NSUInteger)i;
{
    ...
    if ( positionFromStart < shaRange.length )
        return [self.data subdataWithRange: NSMakeRange(shaRange.location + positionFromStart,
                                                        kGITPackIndexSHASize)];
...
- (off_t)packOffsetWithIndex: (NSUInteger)i;
{
    ...
    if ( positionFromStart < offsetsRange.length ) {
        uint32_t value;
        [self.data getBytes: &value range: NSMakeRange(offsetsRange.location + positionFromStart,
                                                       kGITPackIndexOffsetSize)];
        ...
        if ((value & EXTENDED_OFFSET_FLAG) == 0 )
            ...
            [self.data getBytes: &v64 range: NSMakeRange(extendedOffsetRange.location+off64,
                                                         kGITPackIndexExtendedOffsetSize)];
